...
public class OptionalEx1 {
	public static void main(String[] args) {
		...
		OptionalInt optInt1 = OptionalInt.of(0);		// 값을 0으로 넣는 것은 값이 존재하는 뜻임.
		OptionalInt optInt2 = OptionalInt.empty();		// empty() 로 값을 설정하면 값이 존재하지 않는다는 뜻임. 실제로 null값과 같은 것으로 간주함.		
		
		System.out.println(optInt1.isPresent());		// true
		System.out.println(optInt2.isPresent());		// false
			
		System.out.println(optInt1.getAsInt()); 		// 0
		//System.out.println(optInt2.getAsInt());			// NoSuchElementException, null에서 메소드 참조한 것으로 간주
